...
// Tag color configuration ? tune brand colors from each provider logo so they remain distinct
export const TYPE_COLORS: Record<string, TypeColorSet> = {
  ...
  // Kimi logo: bright blue #027AFF (K letter + blue dot)
  kimi: {
    light: { bg: '#dce8ff', text: '#0560cf' },
    dark: { bg: '#003880', text: '#70b5ff' },
  },
  // Gemini logo: multi-tone blue #3186FF (a softer blue)
  gemini: {
    light: { bg: '#e3f2fd', text: '#1565c0' },
    dark: { bg: '#0d47a1', text: '#64b5f6' },
  },
  // Gemini-CLI: same icon as Gemini, distinguished with a deeper navy blue
  'gemini-cli': {
  ...
  // Antigravity logo: multicolor (primary #3789F9 blue + #53A89A teal); use cyan to distinguish
  antigravity: {
  ...
  // iFlow logo: magenta-purple gradient #5C5CFF ? #AE5CFF, leaning magenta to stand apart from Qwen's violet
  iflow: {
...
// Check whether the model is excluded by OAuth
export const isModelExcluded = (
  ...
  return excludedModels.some((pattern) => {
    if (pattern.includes('*')) {
      // Support wildcard matching: escape regex metacharacters first, then treat * as a wildcard
